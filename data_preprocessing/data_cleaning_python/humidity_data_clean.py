...
data_file = 'humidity.csv'  # Update with correct path if needed

# Read CSV, skipping the first two metadata lines and setting column names
...
# Create date column, handling invalid dates (e.g., 29/2 in non-leap years)
...
# Create date(without year) column, ensuring no decimals (e.g., 4/4 instead of 4.0/4.0)
...
result.columns = ['date', 'date(without year)', 'Humidity Value', 'data Completeness']
...
# Save to CSV
result.to_csv('processed_humidity_data.csv', index=False)